 ...
 *    Then execute file DataRoles.java using the java command accompanied by an "java -ea:id.ac.polban.jtk/EngineerRoles/ResearcherRoles DataRoles.java".
...
package id.ac.polban.jtk; // package id.ac.polban.jtk;
   ...
   EngRole.getRole();
        try {
        	...
        	System.out.println(ae.getMessage());
                /*display the error message from method displayRoleDesc()
                * if it encounters AssertionError */
        }
...
class EngineerTools {
    ...
    void removeDataOpsTools() {
        /* make sure the removeAll process works fine by comparing the
        * before and after assertion and try to use try and catch to ensure
        * that removeAll process can catch the error when the method are used
        */
        try {
        	boolean Removecheck = this.EngTools.removeAll(this.DataOpsTools);
        	assert (Removecheck) : "Failed to remove!";
